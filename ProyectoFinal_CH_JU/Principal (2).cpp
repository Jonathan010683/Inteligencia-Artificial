...
Jonnathan Uzhca, Christian Hernandez
*/
...
/*puede definirse como la intersección de todos los conjuntos convexos que contienen X o como el conjunto de todas las combinaciones convexas de puntos en  X .
Un conjunto de puntos se define como convexo si contiene los segmentos de línea que conectan cada par de sus puntos*/
                        convexHull(contours[i],hull[i],true,true);
//una línea que une el punto de inicio y el punto final, luego dibujamos un círculo en el punto más lejano.
...
//se utiliza para dibujar un círculo en cualquier imagen.
                                    circle(img_roi,contours[i][p_end],3,Scalar(0,255,0),2);
                                    count++;
                                }

                            }
//Indentificacion de gesto con la mano
                            if(count==1)
                                strcpy(a,"1");
                            else if(count==2)
                                strcpy(a,"2 ");
...
//Para dibujar los contornos se utiliza la función cv.drawContours . También se puede utilizar para dibujar cualquier forma, siempre que tenga sus puntos límite.
...
//aproxima una curva o un polígono con otra curva / polígono con menos vértices para que la distancia entre ellos sea menor o igual a la precisión especificada
                            approxPolyDP(contours[i],contours_poly[i],3,false);
                            boundRect[i]=boundingRect(contours_poly[i]);
//permiten dibujar el rectángulo dentro de las imágenes y la muestra de video
                            rectangle(img_roi,boundRect[i].tl(),boundRect[i].br(),Scalar(255,0,0),2,8,0);
                            minRect[i].points(rect_point);
                            for(size_t k=0;k<4;k++){
//Para dibujar un lina de color  verdes para la deteccion de la mano
                                line(img_roi,rect_point[k],rect_point[(k+1)%4],Scalar(0,255,0),25,8);
                            }
